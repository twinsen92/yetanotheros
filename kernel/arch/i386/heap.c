...
typedef struct _heap_alloc
{
	uint32_t magic1; /* Magic value for detecting buffer overflows. */

	struct _heap_alloc *prev; /* Previous allocation or NULL if first. */
	struct _heap_alloc *next; /* Next allocation or NULL if last. */
...
/*
    ...
    0               v---heap                      v---first
	[ other memory ][ #1 align_offset_size bytes ][ #1 heap_alloc_t ][ #1 size bytes ][ #2 align_...

	 ...offset_size bytes ][ #2 heap_alloc_t ][ #2 size bytes ]
	                       ^---last                            ^---heap + cur_size    ^
	                                                               heap + heap_size---|
*/

static bool initialized = false;
static const vm_region_t *heap_region;

static spinlock_t spinlock; /* Mutex lock for the heap. */
static size_t heap_size; /* Total heap size in bytes. */
static size_t cur_size;
static void *heap; /* Heap base address. */
static heap_alloc_t *first; /* First allocation. */
static heap_alloc_t *last; /* Last allocation. */
...
static inline bool unsafe_check(heap_alloc_t *alloc)
{
...
static void unsafe_truncate_heap(void)
{
	...
	{
		cur_size -= last->align_offset_size + sizeof(heap_alloc_t) + last->size;
		last = last->prev;
	}
...
vaddr_t kalloc(int mode, uintptr_t alignment, size_t size)
{
	...
	unaligned = (uintptr_t)(heap + cur_size + sizeof(heap_alloc_t));
	offset = alignment - 1 - (size_t)((unaligned + alignment - 1) % alignment);
	v = heap + cur_size + offset + sizeof(heap_alloc_t);

	/* Grow the heap, if necessary. */
	if (heap_size < cur_size + offset + sizeof(heap_alloc_t) + size)
		unsafe_grow_heap(cur_size + offset + sizeof(heap_alloc_t) + size);

	/* Now we can actually access the new memory. */

	/* Build the allocation object. */
	alloc = v - sizeof(heap_alloc_t);
	...
	cur_size += offset + sizeof(heap_alloc_t) + size;
